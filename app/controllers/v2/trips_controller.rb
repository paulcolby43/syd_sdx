class V2::TripsController < ApplicationController
  ...
  def index
    ...
    if params[:driver_id].blank?
      filter = ' {"or": [{"tripStatus": {"eq": "PLANNED" }}, {"tripStatus": {"eq": "STARTED" }} ]} '
    else
      filter = ' {"driverId": {"eq": "' +  params[:driver_id] + '" }, "or": [{"tripStatus": {"eq": "PLANNED" }}, {"tripStatus": {"eq": "STARTED" }} ]} '
    end
...
#          redirect_to search_trips_path
  ...
  # GET v2/trips/1/log_location
  # GET v2/trips/1/log_location.json
  def log_location
    respond_to do |format|
      format.json {
...
#          render json: {error: "Log location failed."}, :status => :bad_request
  ...
  # GET v2/trips/1/locations
  # GET v2/trips/1/locations.json
  def locations
    authorize! :show, :trips
    @trip = Trip.find(current_user.token, params[:id])
  ...
  # GET v2/trips/drivers
  # GET v2/trips/drivers.json
  def drivers
    authorize! :index, :trips
    @dispatch_information = Trip.dispatch_info_by_user_guid(current_user.token)
    @trips = Trip.all_by_user(@dispatch_information)
